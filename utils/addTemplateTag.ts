...
export const addTemplateTag = (construct: Construct, tag: string) => {
  ...
  const baseTelemetry = 'Mult-tenant-SaaS'
...
const appendTagToDescription = (
  ...
  if (existingDescription.includes('(tag:')) {
    ...
    const existingTags = existingDescription
    ...
    if (!existingTags.includes(newTag)) {
      ...
      const newDescription = `${existingDescription.substring(
        0,
        startIndex
      )}${existingTags.join(', ')})`
